...
bool search_qins_files(fs::path rootdir, std::unordered_map<string, fs::path>& imgindex, std::unordered_map<string, PairwiseData>& paramindex)
{
	...
	if (imgindex.size() != imglist.size())
		logg::warn("Warning: images with same name detected!");
	...
	if (paramindex.size() != paramindex.size())
		logg::warn("Warning: transformations with same name detected!");
...
cxxopts::Options parseOptions(std::string exepath="")
{
    ...
    options.add_options()("i,input_folder", "input folder", cxxopts::value<std::string>())
...
int main(int argc, char** argv)
{
    ...
    if (outfilename.is_absolute()) outfilepath = outfilename;
    else outfilepath = rootdirpath / outfilename;
	GRAPH_TYPE type;
