...
as a result, the design matrix can be built in a more structured way to allow the following possible solutions:
	...
	Equivalent to  Gauss-Seidel, the matrix structure of the normal matrix guarantees the convergence (Diagonally Dominant).Extremely memory-friendly,and conceptually has higher numerical precision 
	, for linear, it does not require For non-linear F_ij(P_i,P_j,x_i,x_j), unproven, should have high chance of convergence,Although the norm of jacobian may not be bounded, A factor n could be added to the 
	datum to for a contraction mapping to get obtain a Cauchy sequence.
...
protected:
	...
	std::vector<int> _RefNode_List;                 // this is the reference Node that define the datum, its parameter will never change, if it has multiple, the network is not fully connected and has several models. if this is empty (not initialized externally),the program will initialize this using the Con_Graph. 
	double _epslon;
