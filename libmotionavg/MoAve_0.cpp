...
struct Translate3WithPriorAD {
  ...
  bool operator()(const T* v0, const T* v1, T* residuals) const {
    T tem[3];
    for (int i = 0; i < 3; ++i) tem[i] = v0[i] - v1[i] - xfm[i];
...
bool MoAve_0::_LocalSolver(int index_i) {
  ...
  for (int p = index_i; p <= index_i; p++) {
    ...
    bool skip_ = false;  // no enough measurement, skip it.
    for (int p2 = 0; p2 < _num_para_per_node; p2++)
      if (temWeightVec[p2] < 1e-5) skip_ = true;
    if (skip_) continue;
    for (int p2 = 0; p2 < _num_para_per_node; p2++) _Node_Para_list_buf[p][p2] = temvec[p2] / temWeightVec[p2];  // averaging the current updates
