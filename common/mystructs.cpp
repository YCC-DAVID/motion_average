...
namespace motionavg {
	...
	bool AffineModel::load(std::string p) {
		...
		if (line.compare(0, 10, "affine parameter pixels", 10) == 0)
		{
			...
			{
				for (int i = 0; i < 6; ++i)
					pixelParam[i] = std::stod(tokens[i]);
			}
			else { succ = false; }
		}
		else { succ = false; }
	...
	void XfmGraph::rebase(const double const* newbase) {
		...
		std::copy_n(newbase, 6, newbaseCopy);
		XfmInv(newbase, invNewBase);
...
std::ostream& operator<<(std::ostream& ofs, const motionavg::AffineModel& m)
{
	...
	ofs << " PixParam " << m.pixelParam[0] << " " << m.pixelParam[1] << " " << m.pixelParam[2] << " "
		<< m.pixelParam[3] << " " << m.pixelParam[4] << " " << m.pixelParam[5] << "\n";
	ofs << " GeoParam " << m.geoParam[0] << " " << m.geoParam[1] << " " << m.geoParam[2] << " "
		<< m.geoParam[3] << " " << m.geoParam[4] << " " << m.geoParam[5] << "\n";
	return ofs;
}
...
std::istream& operator>>(std::istream& ifs, motionavg::XfmGraph::Node& n) {
	...
	ifs >> n.name >> dummy >> n.path >> dummy
		>> n.left >> dummy >> n.top >> dummy >> n.right >> dummy >> n.bottom >> dummy;
	for (int i = 0; i < 6; ++i)
		ifs >> n.poseXfm[i] >> dummy;
	return ifs;
}
...
std::istream& operator>>(std::istream& ifs, motionavg::PoseGraph::Edge& e) {
	...
	for (int i = 0; i < 6; ++i)
		ifs >> e.regXfm[i] >> dummy;
	for (int i = 0; i < 36; ++i)
		ifs >> e.covXfm[i] >> dummy;
	return ifs;
}
...
std::ostream& operator<<(std::ostream& ofs, const motionavg::BundleGraph::Edge& e) {
	...
	for (size_t i = 0; i < e.tiepoints.size(); ++i)
		ofs << e.tiepoints[i].target[0] << ' ' << e.tiepoints[i].target[1] << ' '
		<< e.tiepoints[i].source[0] << ' ' << e.tiepoints[i].source[1] << '\n';
	return ofs;
}
...
std::istream& operator>>(std::istream& ifs, motionavg::BundleGraph::Edge& e) {
	...
	{
		ifs >> e.tiepoints[i].target[0] >> dummy >> e.tiepoints[i].target[1] >> dummy >>
			e.tiepoints[i].source[0] >> dummy >> e.tiepoints[i].source[1] >> dummy;
	}
	return ifs;
}