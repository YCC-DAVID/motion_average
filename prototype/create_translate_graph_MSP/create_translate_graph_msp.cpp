...
struct ReprojectError {
  ReprojectError(Eigen::Matrix3d K, const float img_x0, const float img_y0, Eigen::Vector3d cam_pt) 
      : Intr_K(K), Img_pt(img_x0, img_y0), Cam_pt(cam_pt) {} 
  template <typename T>   
  bool operator()(const T* const posi , const T* const Rot, T* residual) const {
    
    /*Eigen::Map<const Matrix<T, 3, 1>> Cam_pt_(Cam_pt);
    Eigen::Map<const Matrix<T, 3, 1>> cam_trans_(cam_trans)*/
    Eigen::Map<const Eigen::Matrix<T, 3, 1>>  cam_trans(posi);
    Eigen::Quaternion<T> Quan_R(Rot[0],Rot[1],Rot[2],Rot[3]);
    T quaw = Quan_R.w();


    Eigen::Matrix<T, 3, 1> Cam_pt_ = Cam_pt.template cast<T>();
    Eigen::Matrix<T, 3, 1> trans = Cam_pt_ - cam_trans;
    Eigen::Matrix<T, 3, 1> tgtC_3d;
    //T tgtC_3d;
    Eigen::Matrix<T, 3, 3> Intr_K_ = Intr_K.template cast<T>();
    Eigen::Matrix<T, 2, 1> Img_pt_ = Img_pt.template cast<T>();
    T quat[4];
    T depth;
    for (int i = 0; i < 4; i++) quat[i] = Quan_R.coeffs().data()[i];
    tgtC_3d = Quan_R * trans;
   // ceres::QuaternionRotatePoint(Quan_R.coeffs().data(), trans.data(), tgtC_3d.data());
    tgtC_3d = Intr_K_ * tgtC_3d;
    depth = tgtC_3d[2];
    tgtC_3d = tgtC_3d / depth;
    
    residual[0] = tgtC_3d[0]  - Img_pt_[0];
    residual[1] = tgtC_3d[1]  - Img_pt_[1];
    return true;
  }
  
  private:
  Eigen::Matrix3d Intr_K;
...
bool create_edge_minimize_reproject_error2(const XYZGrid* srcTmpXYZ, const XYZGrid* tgtTmpXYZ, const QinPose& srcPose, const QinPose& tgtPose, const float* matches, int numpts, float* offset_n_weight,
  ...
  std::vector<cv::Point2f> Matchpointsimg;
  ...
  for (int k = 0; k < numpts; k++)  // k-th matched points
  {
    ...
    if (!valid1) continue;
    // sample according image to read the 3D points in the coordinate which origin point same with camera coordinate and orientation same with world coordinate
    ...
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 3; ++j) {
        Rot_t[i * 3 + j] = srcR(i, j);
      }
    }
    ceres::RotationMatrixToQuaternion(Rot_t, Quan_t);
    Eigen::Quaterniond quan_t(srcR);
    double test = quan_t.w();
    quan_t.normalize();
 
    Eigen::Vector3d objpt_src_t = quan_t*objpt_src;
    Eigen::Vector3d objpt_src_t2 = quan_t.conjugate() * objpt_src;
    Eigen::Vector3d reproj=srcK*objpt_src_t;
    Eigen::Vector3d reproj_ = reproj / reproj[2] - Eigen::Vector3d(srcX,srcY,1);*/


    Matchpoints3d.push_back(cv::Point3f(objpt_src_[0], objpt_src_[1], objpt_src_[2]));
    Matchpointsimg.push_back(cv::Point2f(tgtX, tgtY));
 ...
 cv::Mat distCof(1, 5, CV_64F);
  ...
  bool success = cv::solvePnP(Matchpoints3d, Matchpointsimg, IntK, distCof, rvec, tvec, true);
 if (success)
   cout << "cv2 solve success" << endl;
 else
   cout << "cv2 solve failed" << endl;
 ...
 cv::projectPoints(Matchpoints3d, rvec, tvec, IntK, distCof, cvReprj_px);
 std::vector<cv::Point2f> cvReprj_diff;
 cv::Point2f cvReprj_diff_sum(0, 0);
 for (size_t i = 0; i < cvReprj_px.size(); ++i) {
   cv::Point2f point_diff = Matchpointsimg[i] - cvReprj_px[i];
 ...
 for (size_t i = 0; i < ceReprj_px.size(); ++i) {
   cv::Point2f point_diff = Matchpointsimg[i] - ceReprj_px[i];
  ...
  oss << "sum of Ceres reproject error: [" << ceReprj_diff_sum.x << "," << ceReprj_diff_sum.y << "] with " << num_measure << " points, the ave error: [" << ceReprj_diff_sum.x / num_measure << ","
      << ceReprj_diff_sum.x / num_measure << "]";
  logger->info(oss.str());
  oss.str("");
  oss.clear();
  ...
  if (ceReprj_diff_sum.x / num_measure > 10 || ceReprj_diff_sum.y / num_measure > 10 ) {
...
//}
//
//bool create_edge_from_corrspondence_pts(const XYZGrid* srcTmpXYZ, const XYZGrid* tgtTmpXYZ, const QinPose& srcPose, const QinPose& tgtPose, const float* matches, int numpts, float* offset_n_weight,
//                                           float invsigmasq_2, int minimum_matches, double* xfm, double* cov, spdlog::logger* logger = nullptr) {
//  // calcualting the reproject error
//  auto srcK = srcPose.GetK();
//  auto srcR = srcPose.GetR();
  ...
  for (int i = 0; i < numimg; ++i)  // i-th image
  {
    ...
    for (int j : decoded_neighbors[i]) {
      ...
      for (auto& p : xyzgrid_pairs) {
        ...
        logger.info("Edge[{},{}] Dealing[{},{}]... ", e.source, e.target,p.first, p.second);
