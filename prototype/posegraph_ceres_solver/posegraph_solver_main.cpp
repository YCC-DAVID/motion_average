...
struct Additive_PoseError {
	...
	Additive_PoseError(const double* data)
	{
	...
	*/
					...
					T* residuals) const
	{
		...
		for (int i = 0; i < 6; ++i)
			residuals[i] = rel01[i] - xfm[i];

		residuals[1] *= T(LINEAR_PART_WEIGHT);
		residuals[2] *= T(LINEAR_PART_WEIGHT);
		residuals[4] *= T(LINEAR_PART_WEIGHT);
		residuals[5] *= T(LINEAR_PART_WEIGHT);
...
int main(int argc, char** argv)
{
	...
	fs::path output_graph_path = inputdir / (inputname+"_ceres.txt");
	...
	for (size_t i = 0; i < graph.edges.size(); ++i)
	{
		...
		ceres::CostFunction* cost_function = Compositional_PoseError::Create(e);
